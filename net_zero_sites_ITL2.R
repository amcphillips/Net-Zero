...
sites <- left_join(sites, ITL2, by = c("ladcd" = "LAD23CD"))
...
write_excel_csv(summary, "outputs/ITL2_all_sites.csv")
...
ITL2_GVA_per_job_all_sites <- sites %>%
...
write_excel_csv(ITL2_GVA_per_job_all_sites, "outputs/ITL2_all_sites_GVA_job.csv")


map_data <- 
  right_join(summary, ITL2_boundaries, by = c("ITL221NM" = "ITL221NM")) %>%
  replace(is.na(.), 0) %>%
  distinct()
...
ggsave("outputs/ITL2_all_sites_GVA.png")
...
ggsave("outputs/ITL2_all_sites.png")
...
ggsave("outputs/ITl2_all_sites_employees.png")

map_data2 <- 
  right_join(ITL2_GVA_per_job_all_sites, ITL2_boundaries, by = c("ITL221NM" = "ITL221NM")) %>%
  replace(is.na(.), 0) %>%
  distinct()
...
ggsave("outputs/ITL2_all_sites_GVA_per_job.png")
